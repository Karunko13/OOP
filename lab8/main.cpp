...
int main() {
  ...
  printGameObj(s2);
  printPtr(dynamic_cast<Character*>(s2)); //valid ptr
  printPtr(dynamic_cast<Hurting*>(s2)); //null ptr
    
  Bomb* d2= new Bomb(HP(10), "id2"); // odbiera graczowi 10 hp
  printGameObj(d2);
  printPtr(dynamic_cast<Hurting*>(d2)); //valid ptr
  printPtr(dynamic_cast<Character*>(d2)); //null ptr
